...
export class Playwright {
    ...
    model: string;
    verbose: boolean;
    constructor({ apiKey, model, verbose = true }: { apiKey: string, model: string, verbose?: boolean }) {
        this.apiKey = apiKey;
        this.model = model;
    ...
    async #getThoughtAndCommands() {
        const openaiResponse = await this.#openAIRequest([
            ...
            {
                role: "user",
                content: 'Determine which next command to use, ensuring that the "args" field is always populated with the necessary arguments, and respond using the format specified above.'
            }
        ], mainResponseFormat, mainResponseFormat[0].name);
    ...
    async #interactWithPage(page: Page, task: string) {
        ...
        const taskResponse = await this.#openAIRequest(taskMessage, taskArrResponseFormat, taskArrResponseFormat[0].name)
        const taskArr = JSON.parse(taskResponse.function_call.arguments).tasks
        if (this.verbose == true) { console.log("GOALS: ", taskArr) };
        const history = handleHistory(taskArr);
        history.map((element) => { this.#setHistory(element) });
        ...
        while (1) {
            ...
            if (this.verbose == true) { console.log(parsedOpenaiResponse) }
            ...
            if (command.name == "finish") {
                ...
                this.#setHistory({ role: "assistant", content: `Command finish args: ${command?.args || "NO ARGUMENTS PROVIDED"}` });
                this.#setHistory({ role: "system", content: `I need to insure that all the commmands is finished or Not?` });
                continue;
            }
            ...
            if (command.name == "goToLink") {
                try {
                    ...
                    this.#setHistory({ role: "system", content: `Command ${command.name} returned: Success` });
                } catch (error: any) {
                    this.#setHistory({ role: "system", content: `Args: ${command.args}. Error: ${error.message + error.stack}` });
                }
                continue;
            }

            if (command.name == "findInPage" || commandRunningAttempt >= 3) {
                const findInPagePrompt = await findInPage(page, JSON.stringify(parsedOpenaiResponse));
                const findInPageRespones = await this.#openAIRequest([
                    { role: "system", content: findInPagePrompt },
                    { role: "user", content: "Try finding something in the html body content of the current webpage. use this content to figure out your next step. The task should be written question explaining what you want to find. You can find something like css selector, classes, link, tag, etc. So we can locate the element" }
                ]);
                if (this.verbose == true) { console.log(findInPageRespones.content); }
            ...
            ], playwrightCodeResponseFormat, playwrightCodeResponseFormat[0].name);
            const playwrightCode = JSON.parse(playwrightCodeResponse.function_call.arguments).code;
            if (this.verbose == true) { console.log("Code To Run: ", playwrightCode); }
    ...
    async call({ task, url, headless = true }: { task: string, url?: string, headless?: boolean }) {
        if (!this.apiKey || this.apiKey == undefined) {
            console.log("Please provide apiKey, You can find you apikey here https://platform.openai.com/")
            process.exit();
        }
